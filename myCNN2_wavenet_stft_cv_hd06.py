...
import sys
from myLib_wavenet_stft_cv_hd06 import *
...
import pdb; pdb.set_trace()  # breakpoint 88f99606 //
rand_words = np.load('rand_words.npy')
for cv in range(4):
    sess.run(tf.global_variables_initializer())
    summary_writer = tf.summary.FileWriter(LOG_DIR, sess.graph)
    testset = get_batch(ecog, spkr, cv, rand_words,start_ind, mode = 'test',seg_length=width-64/4, batch_size=32, threshold = 0.0)
    for i in range(3000): #820
        batch = get_batch(ecog, spkr, cv, rand_words,start_ind, mode = 'train',seg_length=width-64/4, batch_size=32, threshold = 0.0)
        ...
        if i%50== 49:
            ...
            batch = get_batch(ecog, spkr, 4, rand_words,start_ind, mode = 'train',seg_length=width-64/4, batch_size=32, threshold = 0.0)
            # batch = get_batch(ecog_train, spkrspec_train,filter_width=100,seg_length=width-64/4, batch_size=32,threshold = 0.0)
