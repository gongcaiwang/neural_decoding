...
bad_samples = np.asarray(range(long(2.14e5/32),long(2.2e5/32))+range(long(5.28e5/32),long(5.42e5/32))+range(long(0),long(3557/32)) + range(long(8.5e5/32),long(8.6e5/32)))
...
def read_all_data(): 
	...
	return net, nst, nett, nstt, spkrspec_train,spkrspec_test, label_train,label_test,start_ind_train,start_ind_test,target_train,target_test,peseudo_train,peseudo_test, word_window_train,word_window_test
...
def get_batch(ecog, spkr,word_window, threshold = 1.0, t_scale = 1, seg_length=160, batch_size=10):


	loop_idx = 0
	# t_scale = 8 # ratio of t_speech/t_ecog, currently 26701/26701=1
	n_delay_1 = 64/4#28 # samples
	n_delay_2 = 64/4#128/4#92#120#92 # samples 
