...
def mu_law_decode(output, quantization_channels):
    ...
    mu = quantization_channels - 1
    # Map values back to [-1, 1].
    signal = 2 * (output.astype(np.float32))
    # Perform inverse of mu-law transformation.
    magnitude = (1. / mu) * ((1 + mu)**np.abs(signal) - 1)
    return np.sign(signal) * magnitude
...
def get_batch(ecog, spkr, threshold = 0.8, t_scale = 4, seg_length=160, batch_size=10): #t_scale = 32


	loop_idx = 0
	# t_scale = 8 # ratio of t_speech/t_ecog, currently 26701/26701=1
	n_delay_1 = 28*8 # samples
	n_delay_2 = 92*8#120#92 # samples
	...
	while (loop_idx < batch_size):
		...
		seg_energy = librosa.feature.rmse(y = np.squeeze(spkspec_seg), frame_length = seg_length,hop_length = seg_length)
