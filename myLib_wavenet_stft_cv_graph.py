...
def read_all_data(): 
	...
	ecogdata = h5py.File(DATA_DIR+'graph_rep_HD01_select_NOspace.mat','r') 
	ecog = np.asarray(ecogdata['xx'])[:,:24]
	...
	ecog = np.maximum(np.minimum(ecog,10),-5)
	...
	ecog_data_hd06 = h5py.File(DATA_DIR+'graph_rep_HD06_select_NOspace.mat','r') 
	ecog_hd06 = np.asarray(ecog_data_hd06['xx'])[:,:24]
	# ecog_data_hd06 = h5py.File(DATA_DIR+'graph_rep_selected_nodes.mat','r') 
	# ecog_hd06 = np.asarray(ecog_data_hd06['xx']) 
	# ecog_hd06 = ecog_hd06[:,channel_correlation_index[-16:]]
	ecog_hd06 = np.maximum(np.minimum(ecog_hd06,10),-5)
	ecog_test = signal.decimate(ecog_hd06[int(1e5):int(9e5)],32,ftype='fir',axis=0)
	statics_ecog = ecog_test.mean(), np.sqrt(ecog_test.var()), ecog_test.mean(axis=0,keepdims=True), np.sqrt(ecog_test.var(axis=0, keepdims=True))
	ecog_hd06 = (signal.decimate(ecog_hd06,32,ftype='fir',axis=0)-statics_ecog[2])/statics_ecog[3]
	sp_ind = ecog_hd06.shape[0]

	ecog_data_hd06sp = h5py.File(DATA_DIR+'graph_rep_HD06sp_select_NOspace_simMselfCompute.mat','r') 
	ecog_hd06sp = np.asarray(ecog_data_hd06sp['xx'])[:,:24]
	# ecog_data_hd06sp = h5py.File(DATA_DIR+'graph_rep_selected_nodes.mat','r') 
	# ecog_hd06sp = np.asarray(ecog_data_hd06sp['xx']) 
	# ecog_hd06sp = ecog_hd06sp[:,channel_correlation_index[-16:]]
	ecog_hd06sp = np.maximum(np.minimum(ecog_hd06sp,10),-5)
	ecog_test = signal.decimate(ecog_hd06sp[int(144667/8):int(7571729/8)],32,ftype='fir',axis=0)
	statics_ecog = ecog_test.mean(), np.sqrt(ecog_test.var()), ecog_test.mean(axis=0,keepdims=True), np.sqrt(ecog_test.var(axis=0, keepdims=True))
	ecog_hd06sp = (signal.decimate(ecog_hd06sp,32,ftype='fir',axis=0)-statics_ecog[2])/statics_ecog[3]

	ecog_hd06 = np.concatenate([ecog_hd06,ecog_hd06sp],axis=0)



	# ecogdata = h5py.File(DATA_DIR+'graph_rep_HD01_select_down.mat','r') 
	# ecog = np.asarray(ecogdata['xx'])
	# # ecogdata = h5py.File(DATA_DIR+'graph_rep_selected_nodes.mat','r') 
	# # ecog = np.asarray(ecogdata['xx']) 
	# # ecog = ecog[:,channel_correlation_index[-16:]]
	# # ecog = np.maximum(np.minimum(ecog,10),-5)
	# # ecog_test = ecog[int(2e5/32):int(10e5/32)]
	# # statics_ecog = ecog_test.mean(), np.sqrt(ecog_test.var()), ecog_test.mean(axis=0,keepdims=True), np.sqrt(ecog_test.var(axis=0, keepdims=True))
	# # ecog = (signal.decimate(ecog,32,ftype='fir',axis=0)-statics_ecog[2])/statics_ecog[3]
	# sp_ind = ecog.shape[0]

	# ecog_data_hd06 = h5py.File(DATA_DIR+'graph_rep_HD06_select_down.mat','r') 
	# ecog_hd06 = np.asarray(ecog_data_hd06['xx'])
	# # ecog_data_hd06 = h5py.File(DATA_DIR+'graph_rep_selected_nodes.mat','r') 
	# # ecog_hd06 = np.asarray(ecog_data_hd06['xx']) 
	# # ecog_hd06 = ecog_hd06[:,channel_correlation_index[-16:]]
	...
	# ecog = np.concatenate([ecog,ecog_hd06],axis=0)
	...
	# spkrspec_test = spkr_test
	spkrspec_test,_,_ = normmat(spkr_test)
	spkr = np.concatenate([spkrspec_train,spkrspec_test],axis=0)

	spkdata = h5py.File(DATA_DIR+'TF32_HD06.mat','r') 
	spkr_hd06 = np.asarray(spkdata['TF32']['TFlog'])
	spkr_hd06,_,_ = normmat(spkr_hd06)

	spkdata_sp = h5py.File(DATA_DIR+'TF32_HD06_sp.mat','r') 
	spkr_hd06sp = np.asarray(spkdata_sp['TF32']['TFlog'])
	spkr_hd06sp,_,_ = normmat(spkr_hd06sp)

	spkr_hd06 = np.concatenate([spkr_hd06,spkr_hd06sp],axis = 0)

	# spkdata_sp = h5py.File(DATA_DIR+'TF32_HD06_sp.mat','r') 
	# spkr_hd06 = np.asarray(spkdata_sp['TF32']['TFlog'])
	# spkr_hd06,_,_ = normmat(spkr_hd06)

	# spkr = np.concatenate([spkr,spkr_hd06],axis = 0)
	...
	end_ind = scipy.io.loadmat(DATA_DIR+'Events.mat')['Events']['offset'][0]
	end_ind = np.asarray([end_ind[i][0,0] for i in range(end_ind.shape[0])])#+697850
	end_ind = end_ind

	start_ind_hd06 = scipy.io.loadmat(DATA_DIR+'Events_HD06.mat')['Events']['onset'][0]
	start_ind_hd06 = np.asarray([start_ind_hd06[i][0,0] for i in range(start_ind_hd06.shape[0])])+697850
	start_ind_hd06 = start_ind_hd06[:100]

	end_ind_hd06 = scipy.io.loadmat(DATA_DIR+'Events_HD06.mat')['Events']['offset'][0]
	end_ind_hd06 = np.asarray([end_ind_hd06[i][0,0] for i in range(end_ind_hd06.shape[0])])+697850
	end_ind_hd06 = end_ind_hd06[:100]

	start_ind_hd06sp = scipy.io.loadmat(DATA_DIR+'Events_HD06_sp.mat')['Events']['onset'][0]
	start_ind_hd06sp = np.asarray([start_ind_hd06sp[i][0,0] for i in range(start_ind_hd06sp.shape[0])])+sp_ind*256
	start_ind_hd06sp = start_ind_hd06sp[:100]

	end_ind_hd06sp = scipy.io.loadmat(DATA_DIR+'Events_HD06_sp.mat')['Events']['offset'][0]
	end_ind_hd06sp = np.asarray([end_ind_hd06sp[i][0,0] for i in range(end_ind_hd06sp.shape[0])])+sp_ind*256
	end_ind_hd06sp = end_ind_hd06sp[:100]

	start_ind_hd06 = np.concatenate([start_ind_hd06,start_ind_hd06sp],axis = 0)
	end_ind_hd06 = np.concatenate([end_ind_hd06,end_ind_hd06sp],axis = 0)
	# start_ind = np.concatenate([start_ind,start_ind_hd06],axis=0)
	# end_ind = np.concatenate([end_ind,end_ind_hd06],axis=0)
	...
	return ecog,spkr,ecog_hd06,spkr_hd06, start_ind,end_ind,start_ind_hd06,end_ind_hd06#,word_window_train,word_window_test
...
def get_batch(ecog, spkr,ecog_hd06, spkr_hd06, start_ind,start_ind_hd06, mode, threshold = 1.0, t_scale = 1, seg_length=160, batch_size=10):

	n_delay_1 = 64/4#28 # samples
	n_delay_2 = 128/4#92#120#92 # samples 

	ecog_train = ecog_hd06
	ecog_test = ecog
	spkr_train = spkr_hd06
	spkr_test = spkr

	start_ind_train = start_ind_hd06
	start_ind_test = start_ind

	if mode == 'train':
		# x = np.asarray(range(start_ind.shape[0])) 
		x = np.asarray(range(start_ind_train.shape[0])) 
		# x = np.concatenate([rand_words[0:cv*test_num],rand_words[(cv+1)*test_num:]])
	elif mode =='test':
		# x = np.asarray(range(start_ind_hd06.shape[0])) 
		x = np.asarray(range(start_ind_test.shape[0])) 
	...
	if mode == 'train': 
		...
		indx = np.maximum(start_ind_train[ind]/256 - np.random.choice(64,batch_size),0)
		for i in range(batch_size):
			ecog_batch[i] = ecog_train[indx[i]+n_delay_1:indx[i]+seg_length+n_delay_2]
			spkr_batch[i] = spkr_train[indx[i]:indx[i]+seg_length]

		# indx = start_ind[ind]/256

	elif mode=='test':

		# indx = np.maximum(start_ind_hd06[x]/256 - 32,0)
		indx = np.maximum(start_ind_test[x]/256 - 32,0)

		ecog_batch = np.zeros((x.size,seg_length+n_delay_2-n_delay_1 ,ecog.shape[1]))
		spkr_batch = np.zeros((x.size, seg_length,32))

		for i in range(x.size):
			ecog_batch[i] = ecog_test[indx[i]+n_delay_1:indx[i]+seg_length+n_delay_2]
			spkr_batch[i] = spkr_test[indx[i]:indx[i]+seg_length]
