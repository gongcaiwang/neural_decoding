...
import sys
from myLib_wavenet_stft_cv_hd06_encode import *
...
def network_wavenet(input_batch, keep_prob, train_phase, reuse=None, name='G'):
    with tf.variable_scope('generator_common_layers',reuse=reuse):
        current = conv(input_batch,num_outputs=depth,kernel_size=100, scope = 'out', activation_fn=None,stride=1,padding='SAME')
...
losses = tf.reduce_mean((tf.reduce_mean((x-pred)**2,axis=-1))**0.5) + L2_REG_WEIGHT * l2_loss 
...
losses_test = tf.reduce_mean((x - pred)**2)
...
for cv in range(4):
    ...
    testset = get_batch(ecog, spkr, cv, rand_words,start_ind, mode = 'test',seg_length=width, batch_size=32, threshold = 0.0)
    for i in range(3000): #820
        batch = get_batch(ecog, spkr, cv, rand_words,start_ind, mode = 'train',seg_length=width, batch_size=32, threshold = 0.0)
        ...
        if i%50== 49:
            ...
            batch = get_batch(ecog, spkr, 4, rand_words,start_ind, mode = 'train',seg_length=width, batch_size=32, threshold = 0.0)
            # batch = get_batch(ecog_train, spkrspec_train,filter_width=100,seg_length=width-64/4, batch_size=32,threshold = 0.0)
            ...
            gt_save_test = testset[0]
            gt_save_train = batch[0]
