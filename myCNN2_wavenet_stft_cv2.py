...
import sys
from myLib_wavenet_stft_cv2 import *
...
#         current = res_block_dis(current, num_outputs=32,kernel_size=4,train_phase=train_phase, scope = 'res2', rate = 1,mode='generator')
#         current = res_block_dis(current, num_outputs=32,kernel_size=4,train_phase=train_phase, scope = 'res3', rate = 1,mode='generator')
#         current = res_block_dis(current, num_outputs=32,kernel_size=4,train_phase=train_phase, scope = 'res4', rate = 1,mode='generator')
#         current = res_block_dis(current, num_outputs=32,kernel_size=4,train_phase=train_phase, scope = 'res5', rate = 1,mode='generator')
#         current = res_block_dis(current, num_outputs=32,kernel_size=4,train_phase=train_phase, scope = 'res6', rate = 1,mode='generator')
#         current = res_block_dis(current, num_outputs=32,kernel_size=4,train_phase=train_phase, scope = 'res7', rate = 1,mode='generator')
#         current = res_block_dis(current, num_outputs=32,kernel_size=4,train_phase=train_phase, scope = 'res8', rate = 1,mode='generator')
...
# losses = tf.reduce_mean((y_ - pred)**2*(word_window+1.0)) + L2_REG_WEIGHT * l2_loss + l1_loss
...
ecog,spkr,label_train,label_test,start_ind,target_train,target_test,peseudo_train,peseudo_test,word_window_train,word_window_test,word_window_ = read_all_data()
rand_words = np.load('rand_words.npy')
for cv in range(3):
    sess.run(tf.global_variables_initializer())
    summary_writer = tf.summary.FileWriter(LOG_DIR, sess.graph)
    testset = get_batch_show(ecog, spkr, cv, start_ind ,seg_length=width-64/4)

    for i in range(3000): #820
        batch = get_batch(ecog, spkr, word_window_, cv,seg_length=width-64/4, batch_size=32, threshold = 0.0)
        ...
        if i%50== 49:
            ...
            print("Avg Test Losses %g, CC %g" %(test_losses,cor_coef(gt_save_test,final_result)))
