...
import sys
from myLib_wavenet_stft_cv_graph import *
...
ecog,spkr,ecog_hd06,spkr_hd06, start_ind,end_ind,start_ind_hd06,end_ind_hd06 = read_all_data()
rand_words = np.load('rand_words.npy')
for cv in range(1):
    sess.run(tf.global_variables_initializer())
    summary_writer = tf.summary.FileWriter(LOG_DIR, sess.graph)
    testset = get_batch(ecog, spkr,ecog_hd06, spkr_hd06, start_ind,start_ind_hd06, mode = 'test',seg_length=width-64/4, batch_size=32, threshold = 0.0)
    for i in range(3000): #820
        batch = get_batch(ecog, spkr,ecog_hd06, spkr_hd06, start_ind,start_ind_hd06, mode = 'train',seg_length=width-64/4, batch_size=32, threshold = 0.0)
