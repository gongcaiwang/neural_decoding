...
bad_samples = np.asarray(range(long(2.14e5/32),long(2.2e5/32))+range(long(5.28e5/32),long(5.42e5/32)))
...
def read_all_data(): 
	...
	ecogdata = h5py.File(DATA_DIR+'gdat_env3.mat','r') 
	...
	return ecog,spkr, label_train,label_test,start_ind,target_train,target_test,peseudo_train,peseudo_test,word_window_train,word_window_test,word_window


def get_batch(ecog, spkr,word_window, cv, threshold = 1.0, t_scale = 1, seg_length=160, batch_size=10):

	test_segs = np.array([[0,8860],[8860,17414],[17414,26703]])
	test_seg = test_segs[cv]
	...
	word_window_batch = np.zeros((1,seg_length/t_scale,1)) 
	...
	while (loop_idx < batch_size):
		# loop_total += 1
		# seg_index = np.random.choice(range(0,26703- seg_length - 1 - n_delay_2))
		seg_index = np.random.choice(range(0,test_seg[0] - seg_length - 1 - n_delay_2)+ range(test_seg[1],26703 - seg_length - 1 - n_delay_2))
		...
		word_window_seg = word_window[seg_index : seg_index+seg_length]
		...
		if avg_energy > threshold:
			...
			word_window_batch = np.vstack((word_window_batch, word_window_seg[np.newaxis,:,np.newaxis]))
	...
	word_window_batch = word_window_batch[1:]

	return ecog_batch, spkr_batch, word_window_batch

def get_batch_show(ecog, spkr,cv,start_ind,threshold = 1.0, t_scale = 64*4, seg_length=160):
	# t_scale = 8 # ratio of t_speech/t_ecog, currently 26701/26701=1
	...
	indx = start_ind[np.where((start_ind>test_seg[0]) & (start_ind<test_seg[1]))]
	ecog_batch = np.zeros((indx.shape[0],seg_length+n_delay_2-n_delay_1 ,ecog.shape[-1]))
	spkr_batch = np.zeros((indx.shape[0],seg_length,32))
