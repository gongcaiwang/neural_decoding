...
def get_batch(ecog, spkr, cv, rand_words, start_ind, mode, threshold = 1.0, t_scale = 1, seg_length=160, batch_size=10):
	...
	test_num = int(math.ceil(words_num*1.0/5)) # 5 fold cross val
	...
	if mode == 'train': 
		ecog_batch = np.zeros((batch_size,seg_length+n_delay_2-n_delay_1 ,63))
		spkr_batch = np.zeros((batch_size, seg_length,32))
		ind = np.random.choice(x,batch_size)
		indx = np.maximum(start_ind[ind]/256+64 - np.random.choice(128,batch_size),0)
		for i in range(batch_size):
			ecog_batch[i] = ecog[indx[i]+n_delay_1:indx[i]+seg_length+n_delay_2]
		...
		ecog_batch = np.zeros((x.size,seg_length+n_delay_2-n_delay_1 ,63))
