...
def get_batch(ecog, spkr, cv, rand_words, start_ind, mode, filter_width, threshold = 1.0, t_scale = 1, seg_length=160, batch_size=10):
	...
	n_delay_2 = filter_width-1#92#120#92 # samples 
	...
	if mode == 'train': 
		ecog_batch = np.zeros((batch_size,seg_length+n_delay_2,64))
		spkr_batch = np.zeros((batch_size, seg_length,32))
		ind = np.random.choice(x,batch_size)
		indx = np.maximum(start_ind[ind]/256 - np.random.choice(64,batch_size),0)
		for i in range(batch_size):
			ecog_batch[i] = ecog[indx[i]+n_delay_1:indx[i]+seg_length+n_delay_1+n_delay_2]
		...
		ecog_batch = np.zeros((x.size,seg_length+n_delay_2,64))
