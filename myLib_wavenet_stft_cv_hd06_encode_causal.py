...
def read_all_data(): 
	...
	label_mat = scipy.io.loadmat(DATA_DIR+'Events_HD06.mat')['Events']['word'][0]
	labels = []
	unique_labels = []
	...
	target_mat = scipy.io.loadmat(DATA_DIR+'Events_HD06.mat')['Events']['istarget'][0]
	targets = []
	for i in range(label_mat.shape[0]):
	...
	peseudo_mat = scipy.io.loadmat(DATA_DIR+'Events_HD06.mat')['Events']['ispseudo'][0]
	peseudos = []
	for i in range(label_mat.shape[0]):
	...
	return ecog,spkr, label_train,label_test,start_ind,target_train,target_test,peseudo_train,peseudo_test#,word_window_train,word_window_test
...
def get_batch(ecog, spkr, cv, rand_words, start_ind, mode, filter_width, threshold = 1.0, t_scale = 1, seg_length=160, batch_size=10):
	...
	test_num = int(math.ceil(words_num*1.0/4)) # 4 fold cross val

	n_delay_1 = -filter_width+1#28 # samples
	n_delay_2 = 64/4#92#120#92 # samples 
	...
	if mode == 'train': 
		ecog_batch = np.zeros((batch_size,seg_length ,64))
		spkr_batch = np.zeros((batch_size, seg_length-n_delay_1,32))
		ind = np.random.choice(x,batch_size)
		indx = np.maximum(start_ind[ind]/256 - np.random.choice(64,batch_size),0)
		for i in range(batch_size):
			ecog_batch[i] = ecog[indx[i]+n_delay_2:indx[i]+seg_length+n_delay_2]
			spkr_batch[i] = spkr[indx[i]+n_delay_1:indx[i]+seg_length]
		...
		ecog_batch = np.zeros((x.size,seg_length ,64))
		spkr_batch = np.zeros((x.size, seg_length-n_delay_1,32))
