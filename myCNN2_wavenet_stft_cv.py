...
import sys
from myLib_wavenet_stft_cv import *
...
ecog,spkr,label_train,label_test,start_ind,target_train,target_test,peseudo_train,peseudo_test,word_window_train,word_window_test = read_all_data()
rand_words = np.load('rand_words.npy')
for cv in range(5):
    sess.run(tf.global_variables_initializer())
    summary_writer = tf.summary.FileWriter(LOG_DIR, sess.graph)
    testset = get_batch(ecog, spkr, 4, rand_words,start_ind, mode = 'test',seg_length=width-64/4, batch_size=32, threshold = 0.0)
